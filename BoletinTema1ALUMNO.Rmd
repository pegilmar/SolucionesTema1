...
author: "Pedro Gil Martin"
date:  "`r Sys.Date(15/02/2022)`"  #Pondría la fecha del día actual
params:
  lang: ES
...
```{r}
#apartado d
commonsAnimals <- intersect(mammals, Animals2)
commonsAnimals
```
...
```{r}
#apartado e
notcommonsAnimals <- setdiff(Animals2, mammals)
notcommonsAnimals

#Hay 3 animales, en concreto 3 dinosaurios, que son los únicos que no se encuentran en ambas listas.
```
...
```{r}
#apartado f
#Aunque los 3 sean dinosaurios, los datos dejan claras importantes diferencias fisicas entre ellos.
```
...
#Presentan un comportamiento bastante similar y siguen una distribución Gaussiana ya que todos los puntos se encuentran dentro de la zona acotada en color azul.
```
...
3.  La función **library** sin argumentos abre una ventana y muestra las librerías que han sido instaladas.
    ...
    libReturn <- library()
    libReturn
    #Observamos un montón de librerias con su respectiva descripción.

    #apartado b
    head(libReturn$results, n=5)
    #Se muestran los 5 primeros elementos solicitados con sus respectivos títulos.

    #apartado c
    search()
    #Se puede comprobar que en este momento tengo instaladas 24 librerias.
    ```
...
#apartado b
any(is.na(cabbages))
#Por tanto, no hay valores perdidos en el conjunto de datos cabbages.
...
any(is.na(Chile))
#Vemos que en el conjunto de datos Chile si hay valores perdidos.
colSums(sapply(Chile, is.na))
#En concreto vemos que tenemos 295 valores perdidos.
...
#En ambos casos todos los puntos se encuentran dentro de la zona acotada en azul, por tanto podriamos afirmar que siguen una distribución Gaussiana.
```
